	...
	public class DownloadedFileCleanupService : IDownloadedFileCleanupService {
		...
		public void RunCleanup() {
			...
			foreach (DownloadToDelete download in this.completedDownloads.Where(IsDeletable)) {
				...
				if (this.fileManager.TryGetFile(filename, out IFile file) && file.Delete()) {
					this.logger.LogInformation($"File deleted: {filename}");
					ToRemove.Add(download);
				}
				else {
					this.logger.LogWarning($"File cannot be deleted or does not exists: {filename}");
				}
			}
